...
export class OpenfinUtils {
    ...
    public static async open(path: string,
        ...
        if (!this.isRunning()) {
            this.log(path, 'openfin is offline, opening: \n' + windowUrl);
        ...
        const win = new fin.desktop.Window({
            ...
            // hideOnClose: false,
            // backgroundColor: '#FFF'
        }, function(): void { // on window created
    ...
    public static CheckOptions(
        ...
        console.log('options ' + options.width + 'x' + options.height + ' icon=' + options.showTaskbarIcon);
        return options;
    }

    public static log(windowID: string, action: string): void {
        // const winInfo = ''' + windowID + ''';
        const winInfo = 'openfin ' + windowID + ' ';
        console.log(winInfo + ' ' + action);
    }

    public static async logInfo(): Promise<void> {
    ...
    public static notify(title: string, body: string, category: string): void {
