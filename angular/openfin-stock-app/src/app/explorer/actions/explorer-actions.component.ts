...
import { Fdc3Position } from "igniteui-angular-fdc3";    // for defining context of FDC3 message
import { Fdc3Message } from "igniteui-angular-fdc3";     // for receiving FDC3 messages
...
export class ExplorerActionsComponent implements AfterViewInit {
    title = "Explorer for sending FDC3 Messages";
    ...
    public async InitializeFDC3(): Promise<void> {
        ...
        this.FDC3adapter.subscribe("ViewChart");      // used in Chart with FDC3 ViewChart window
        this.FDC3adapter.subscribe("ViewInstrument"); // used in Grid with FDC3 ViewInstrument window
        this.FDC3adapter.subscribe("ViewPosition");   // used in Grid with FDC3 ViewPosition window
        this.FDC3adapter.subscribe("ViewPortfolio");  // used in Grid with FDC3 ViewPosition window
        this.FDC3adapter.subscribe("ViewSector");     // used in Treemap with FDC3 ViewSector window
    ...
    public ReportOpenFinIsMissing(): void {
        let info = ""; // this.messageHistory.nativeElement.textContent;
        info = info + "============================\n";
        info = info + "Warning: OpenFin FDC3 Service is offline. ";
        info = info + "You need to run this application from OpenFin launcher to send FDC3 messages. ";
        info = info + "See instructions in ReadMe.md file \n";
        const old = this.messageHistory.nativeElement.textContent;
        this.messageHistory.nativeElement.textContent = info + old;
    }
